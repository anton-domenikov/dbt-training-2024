with
	paid_orders as (
		...
		from ld_raw.jaffle_shop.orders as orders
		left join
			(
				select
					orderid as order_id,
		...
		left join ld_raw.jaffle_shop.customers c on orders.user_id = c.id
	),
	...
	customer_orders as (
		...
		from ld_raw.jaffle_shop.customers c
		left join ld_raw.jaffle_shop.orders as orders on orders.user_id = c.id
		group by 1
	)
